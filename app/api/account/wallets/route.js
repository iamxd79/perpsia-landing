...
export async function POST(request) {
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/api/account/wallets`, {
      method: "POST",
    ...
    return NextResponse.json(await response.json().catch(() => ({ error: "Invalid wallet response." })), { status: response.status });
