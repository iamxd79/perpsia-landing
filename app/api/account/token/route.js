...
export async function POST(request) {
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/api/account/token`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${internalToken}` }, body: JSON.stringify({ privyAccessToken: body.privyAccessToken }), cache: "no-store", signal: AbortSignal.timeout(10000) });
    return NextResponse.json(await response.json().catch(() => ({ error: "Invalid token response." })), { status: response.status });
  } catch { return NextResponse.json({ error: "Token service is temporarily unavailable." }, { status: 503 }); }
}
