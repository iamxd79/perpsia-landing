...
export async function POST(request) {
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/api/account/preferences`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${internalToken}` }, body: JSON.stringify(body), cache: "no-store", signal: AbortSignal.timeout(8000) });
